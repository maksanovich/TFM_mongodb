...
def test_proxies_in_loop ():
   ...
   for counter, my_ip in enumerate(LIST_P):
      ...
      if proxy_response is None or '200' not in str(proxy_response) or my_ip in proxy_response:
         ...
         LIST_proxy_BAD.append(my_ip)
          
      else: 
   
         print (my_ip + " NOT in --> " + str(proxy_response), flush=True)  
         LIST_proxy_GOOD.append(my_ip)
...
#print ("GOOD proxies --> " + str(test_proxies_in_loop ()[0]), flush=True)
#print ("BAD proxies --> " + str(test_proxies_in_loop ()[1]), flush=True)
